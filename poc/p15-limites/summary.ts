...
export type P15Obs = {
  read: { freshMs: number; cachedMaxMs: number; trigger: string; erros: string[]; pendentesForaDaReautorizacao: string[]; itens: { id: string; status: string; source: string }[] };
...
export function summarizeP15(o: P15Obs) {
  const c1 = { pass: o.read.itens.length >= 11 && o.read.erros.length === 0 && o.read.pendentesForaDaReautorizacao.length === 0, fontes: o.read.itens.length, erros: o.read.erros, pendentes: o.read.itens.filter((i) => i.status === 'pendente').map((i) => i.id) };
