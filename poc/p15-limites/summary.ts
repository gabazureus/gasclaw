...
export function summarizeP15(o: P15Obs) {
  ...
  const porDecisao = o.read.itens.filter((i) => i.status === 'erro' && /faturamento/.test(i.note ?? '')).map((i) => i.id);
  const erros = o.read.erros.filter((id) => !porDecisao.includes(id));
  const c1 = { pass: o.read.itens.length >= 11 && erros.length === 0 && o.read.pendentesForaDaReautorizacao.length === 0, fontes: o.read.itens.length, erros, indisponiveisPorDecisao: porDecisao, pendentes: o.read.itens.filter((i) => i.status === 'pendente').map((i) => i.id) };
