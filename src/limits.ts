...
export function buildLimits(i: LimitsInput): LimitItem[] {
  ...
  return [
    ...
    item('urlfetch', 'Apps Script: chamadas UrlFetch hoje (estimado)', 'chamadas', 'gasclaw', OK, m.urlFetchToday, 100_000, GOOGLE_DAY),
    ...
    item('processes', 'Apps Script: tempo de gatilhos hoje', 'ms', 'google', i.processes, i.processes.ok ? i.processes.value.triggerMsToday : null, 21_600_000, GOOGLE_DAY),
    item('mail', 'Apps Script: destinatários de e-mail hoje', 'destinatários', 'google', i.mail, i.mail.ok ? 1500 - i.mail.value : null, 1500, GOOGLE_DAY),
