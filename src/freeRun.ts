...
import { freeFailures, keyInfo, listModels, noteFreeFailure } from './models';
...
export function freeQuotaNow(apiKey: string | null) {
  ...
  let freeTier = false;
  try {
    freeTier = apiKey ? keyInfo(apiKey).is_free_tier : false;
  } catch {
    freeTier = false; // sem leitura da chave, assume o limite maior e deixa o 429 do provedor decidir
  }
  return { today, perMinute: freePerMinuteMax(u), freeTier };
}
 ...
 */
export function runFree(call: (model: string) => Completion, o: { tools: boolean; apiKey: string | null; now?: number }): FreeCompletion {
  const now = o.now ?? Date.now();
  const q = quota(freeQuotaNow(o.apiKey));
