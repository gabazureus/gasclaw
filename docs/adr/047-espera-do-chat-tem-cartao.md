...
3. **Chat fora do ar**: a espera volta à fila com `notBefore` = agora + 5 min (`PROMPT_RETRY_MS`). As 4
   tentativas (`MAX_ATTEMPTS`) cobrem cerca de 15 min. Esgotadas, a entrega desiste com o motivo. Um destino
   que diverge da autoridade desiste na hora.
4. **Varredura das esperas** (`recoverWaits`), para os runs parados antes deste conserto. Ela usa a MESMA
   ...
   é procurada entre os agentes cadastrados.
...
- Implementação: `waitKey`/`waitLabel` (`src/run.ts`), `settle` (`src/runner.ts`), `scan`/`markPrompted`
  (`src/runStore.ts`), `promptInChat`/`recoverWaits`/`answerOpenAsk`/`durableChatClick` (`src/main.ts`),
