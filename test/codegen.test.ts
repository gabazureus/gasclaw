...
// D7 — SUBSTITUÍDO, E O MOTIVO FICA REGISTRADO AQUI.
//
// Esta seção testava uma contagem de chaves dentro do crivo, que recusava "fonte cortado no meio". A
// primeira geração REAL do Opus mostrou o defeito dela: não tratava regex, e um compositor de CSV
// correto (`/"/g`, `/["\n]/`) era lido como truncado — o `"` dentro da regex abria uma string que
// nunca fechava. A geração foi queimada (US$ 0,108) por esse falso positivo.
//
// E o commit original do D7 errou a direção do risco: dizia que a lacuna de regex "deixava passar um
// corte, do lado seguro". Era o contrário — recusava código BOM, o lado caro.
//
// O corte agora é decidido em `generateSuccessor` pelo `finish_reason` da API (ver
// `test/successor.test.ts`, "corte: decidido pelo finish_reason"). O que sobra aqui é a garantia
// inversa: o crivo NÃO recusa código correto por causa de chave, aspa ou regex no texto.
describe('crivo: código correto não é recusado por causa do texto dentro dele', () => {
  test('chave e aspa dentro de string, comentário, template e REGEX passam', () => {
    for (const bom of [
      ...
      'function doGet(e) { // }\n return 1; }',
      'function doGet(e) { return s.replace(/"/g, \'""\'); }',
      'function doGet(e) { return /["\\n]/.test(s); }',
    ]) {
      expect(checkSuccessorSource(bom).ok).toBe(true);
    }
