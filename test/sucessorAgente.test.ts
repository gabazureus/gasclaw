...
beforeEach(() => {
  ...
  env.route = (url, init) => {
    ...
    if (url === `${URL_SLOT}?action=handover` && init.method === 'post') return { code: 200, body: JSON.stringify(heranca) };
...
describe('crownSuccessor: a coroa — só com o health inteiro; o titular para ANTES, e volta se ela falhar', () => {
  ...
  test('succeed desligado: recusa a coroa (a mesma porta de mayAct que escrever usa)', async () => {
    slotRegistrado(avaliado(5, 5));
    env.props['CAP:f1'] = '[]';
    expect((await motor()).crownSuccessor('SLOT').ok).toBe(false);
    expect(coroou()).toBe(false);
  ...
  test('coroa pela metade com o titular JÁ parado: conclui mesmo assim (é o que termina os dois motores)', async () => {
    slotRegistrado(avaliado(5, 5));
    env.props['RUNTIME_ENABLED'] = 'false';
    self = { ...SELF_OK, enabled: true, trigger: 'active' };
    expect((await motor()).crownSuccessor('SLOT')).toMatchObject({ ok: true });
...
describe('rebaseSuccessor: o mesmo patch sobre o código ATUAL, sem o Opus', () => {
  ...
  // AUDITORIA 2026-09-21: rebase implanta e avaliar gasta OpenRouter — e nenhum dos dois passava por mayAct,
  // então `./gasclaw down`, succeed desligado e o congelamento não os paravam.
  test.each([
    ['motor parado', () => void (env.props['RUNTIME_ENABLED'] = 'false')],
    ['succeed desligado', () => void (env.props['CAP:f1'] = '[]')],
    ['congelamento de emergência', () => void (env.props['CAPS_ENABLED'] = 'false')],
  ])('rebase e avaliação recusam com %s, sem implantar nem gastar', async (_n, preparar) => {
    slotRegistrado({ changes: troca });
    preparar();
    const m = await motor();
    expect(m.rebaseSuccessor('SLOT').ok).toBe(false);
    expect(escrito()).toBeNull();
    expect(m.evaluateSuccessor('SLOT').ok).toBe(false);
    expect(env.fetched('openrouter.ai/api/v1/chat')).toHaveLength(0);
  });
});
...
describe('herança: o pai manda, o filho aceita só a lista fechada', () => {
  ...
  const enviado = () => env.calls.find((c) => c.url === `${URL_SLOT}?action=handover`);
  ...
  test('a coroa HERDA antes de ligar o sucessor', async () => {
    ...
    expect((await motor()).crownSuccessor('SLOT')).toMatchObject({ ok: true });
    const o = ordem();
    expect(o.indexOf('handover')).toBeGreaterThanOrEqual(0);
    expect(o.indexOf('handover')).toBeLessThan(o.indexOf('crown'));
  });
...
describe('no SUCESSOR, a porta `inherit`: só do pai da semente, e só a lista fechada', () => {
  const post = async (parent: string, entries: Record<string, string>) => {
    vi.stubGlobal('GASCLAW_SEED', { bornDisabled: true, parent: 'PAI', agents: [{ name: 'agente-teste', folderId: 'f1' }], at: 1 });
    const m = await import('../src/main');
    const out = m.doPost({ parameter: { action: 'handover' }, postData: { contents: JSON.stringify({ parent, entries }), type: 'application/json' } } as unknown as GoogleAppsScript.Events.DoPost) as unknown as { getContent: () => string };
  ...
  test('corpo ilegível é recusado sem gravar', async () => {
    vi.stubGlobal('GASCLAW_SEED', { bornDisabled: true, parent: 'PAI', agents: [{ name: 'agente-teste', folderId: 'f1' }], at: 1 });
    const m = await import('../src/main');
    const out = m.doPost({ parameter: { action: 'handover' }, postData: { contents: '{lixo', type: 'application/json' } } as unknown as GoogleAppsScript.Events.DoPost) as unknown as { getContent: () => string };
...
// O TESTE QUE FALTOU: a ação `inherit` pelo caminho REAL da CLI (doPost com o segredo). Com a porta do
// sucessor de mesmo nome, ela era interceptada no pai antes do segredo e nunca chegava ao sucessor.
describe('succession inherit pela CLI chega à herança, não à porta', () => {
  test('doPost inherit com o segredo manda a herança ao sucessor', async () => {
    slotRegistrado({ crownedAt: 9 });
    const SEGREDO = 'a'.repeat(64);
    env.props['CLI_SECRET'] = SEGREDO;
    const m = await import('../src/main');
    const out = m.doPost({ parameter: { action: 'inherit', secret: SEGREDO, child: 'SLOT' } } as unknown as GoogleAppsScript.Events.DoPost) as unknown as { getContent: () => string };
    expect(JSON.parse(out.getContent())).toMatchObject({ ok: true });
    expect(env.calls.some((c) => c.url === `${URL_SLOT}?action=handover`)).toBe(true);
  });
});
