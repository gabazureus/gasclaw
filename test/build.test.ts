...
test('o build embute o ambiente e a URL do painel irmão', () => {
  ...
  execSync('node build.mjs', { stdio: 'pipe', env: { ...process.env, OUT_DIR: out, GASCLAW_ENV: 'dev', SIBLING_URL: sibling } });
  const code = readFileSync(join(out, '_motor.js'), 'utf8');
  ...
  execSync('node build.mjs', { stdio: 'pipe', env: { ...process.env, OUT_DIR: neutro, GASCLAW_ENV: '', SIBLING_URL: '' } });
