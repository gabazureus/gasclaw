...
- 🔄 `./gasclaw eval` roda testes automáticos do agente no dev; ferramentas `now` e memória (`memory.save/remove/read`) com lista permitida por agente; aprovação com card de uso único (10 min) e perguntas com botões (em integração).
